...
{{
    ...
    "sample_size_over_10k": 0
        ...
        # Post-Hoc Linear Adjustment
        p = row['ipw_score']
        # Ensure p is strictly between 0 and 1
        p = max(1e-9, min(1 - 1e-9, p))
        logit = math.log(p / (1 - p))
        
        final_logit = logit + 0.3 * causal_id + 0.3 * novel_data
        final_p = 1 / (1 + math.exp(-final_logit))
        
        print(f"  ID {csv_id} (Vanilla Rank {idx+1}) -> Causal: {causal_id}, Novel: {novel_data} | IPW: {p:.4f} -> Hybrid: {final_p:.4f}")
        mapping_df.loc[idx, 'hybrid_score'] = final_p
    ...
    print("\n==================================================")
    print(" STRATEGY B: POST-HOC FEATURE INJECTOR METRICS")
    print("==================================================")
