...
def train_and_predict(X_train, y_train, X_test, y_test, m_param):
    print(m_param)
    models = {
        # regression
        'Lasso': Lasso(),
...
def train_and_pred_value(X_train, y_train, X_test, y_test, m_param):
    random.seed(0)
    print(m_param)
    models = {
        # regression
    ...
    for x in ['B', 'C', 'D', 'E', 'F']:
        ...
        for target in [
            ...
            try:
                ...
                X_train, y_train = smote.fit_resample(X_train0, y_train0[target])
            except Exception:
                smote = SMOTE(sampling_strategy=1, random_state=0, k_neighbors=2)
                X_train, y_train = smote.fit_resample(X_train0, y_train0[target])

            vali = pd.DataFrame()
            for m_param in m_param_list:
                temp, temp_score = train_and_pred_value(X_train, y_train, X_val, y_val[target], m_param)
            ...
            final = pd.DataFrame()
            for m_param in m_param_list:
                temp = train_and_predict(X_train, y_train, X_val, y_val[target], m_param)
                final = pd.concat([final, temp])

            final['model'] = [list(eval(i).keys())[0] for i in list(final.index)]
            best = final.groupby('model')['AUC'].idxmax()
            best = best.dropna()
            print(best)
            ...
            for i in range(len(best)):
                res, score = train_and_pred_value(
                    X_train, y_train, X_test, y_test[target], eval(best.iloc[i])
                )
...
def get_metric_test(y_true, y_pred, y_score):
    ...
    return pd.Series({
           ...
           'precision@K':recall_at_k(y_true,y_score, k=y_true.sum()),
            'LIFT':recall_at_k(y_true,y_score, k=y_true.sum())/y_true.mean(),
